...
export default function SpLocationPage() {
  ...
  const fetchLocations = async (currentPage, currentSearch, isAppend) => {
    ...
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/special-locations`, {
      ...
      toast.error('Failed to load special locations');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      await axios.delete(`${import.meta.env.VITE_API_URL}/special-locations/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
          ...
          <main className="flex-1 overflow-y-auto p-4 sm:p-6">
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 rounded-xl shadow-sm border border-slate-200 gap-4">
                <h1 className="text-2xl font-bold text-slate-800">Special Location Information</h1>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col" style={{ maxHeight: 'calc(100vh - 250px)' }}>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead className="sticky top-0 z-10 bg-slate-50 shadow-sm">
                      <tr className="border-b border-slate-200 text-slate-600 text-sm uppercase tracking-wider">
                        ...
                        <th className="p-4 font-semibold">Special Location Name</th>
                    ...
                    <tbody className="divide-y divide-slate-200">
                      ...
                      {isAdding && (
                        <tr className="bg-blue-50/50">
                          ...
                          <td className="p-4 text-slate-400 text-sm italic">Auto-filled</td>
                          <td className="p-4">
                      ...
                      {locations.map((loc) => (
                        <tr key={loc.ID} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="p-4">
                            {editingId === loc.ID ? (
                              <div className="flex items-center justify-center gap-2">
                                <button onClick={() => handleUpdateSubmit(loc.ID)} className="p-1.5 text-green-600 hover:bg-green-100 rounded-lg transition-colors" title="Save">
                                  <CheckIcon className="w-5 h-5" />
                                </button>
                              ...
                              <div className="flex items-center justify-center gap-3">
                                <button
                                  onClick={() => { setEditingId(loc.ID); setEditLocationNo(loc.locationno); }}
