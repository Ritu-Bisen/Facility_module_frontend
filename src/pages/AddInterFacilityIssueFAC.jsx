...
export default function AddInterFacilityIssueFAC() {
  ...
  const loadFacilities = async () => {
    const res = await getInFacilityTransferFacilities(facilityId);
    if (res.success) {
  ...
  const handleGenerateIssueNo = async () => {
    ...
    try {
      if (parentIssueId) {
        ...
        const res = await generateAndSaveInFacilityTransferIssueNo(facilityId, payload);
        if (res.success) {
          setIssueNo(res.issueNo);
          setHeaderSaved(true);
          localStorage.setItem('currentInFacIssueNo', res.issueNo);
          localStorage.setItem('currentInFacIssueHeaderSaved', 'true');
          
          if (res.issueId || res.data?.issueId) {
            const newId = res.issueId || res.data.issueId;
            setParentIssueId(newId);
            localStorage.setItem('currentInFacIssueId', newId);
          }
          alert("Issue Generated successfully");
        } else {
          alert(res.message || "Failed to generate issue");
        }
      }
    } catch (e) {
      console.error(e);
            ...
            <div className="max-w-7xl mx-auto space-y-6">
              ...
              <div className="space-y-2">
                <div className="-mt-3 mb-1">
                  <button
                    onClick={() => navigate('/inter-facility-issue')}
                ...
                <div className="bg-red-50 border-l-4 border-red-500 p-2.5 rounded-r-lg shadow-sm">
                  <div className="flex items-start">
                    <div className="flex-shrink-0 mt-0.5">
                      <ExclamationTriangleIcon className="h-4 w-4 text-red-500" aria-hidden="true" />
                    </div>
                    <div className="ml-2 text-[11px] text-red-700 font-medium leading-relaxed">
                      <strong className="text-red-800 mr-1">*Note:-</strong>
                      EDL 2019 के अनुसार ही Inter Facility Transfer किया जा सकेगा | (Example: CHC/CH up to 'S' category, PHC/UPHC upto 'P' category, DH/CMHO upto 'T' category and Medical College upto 'M' category.)
                      <span className="ml-2">* विषम परिस्थिति में आवश्यकता अनुसार पत्र upload करते हुए अन्य category के drugs को भी transfer किया जा सकेगा.</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="p-4 space-y-4">
                  <div className="flex flex-wrap items-end gap-4">
                    ...
                    <div className="flex-shrink-0 flex items-center gap-3 mb-0.5">
                      {headerSaved ? (
                        <>
                          <button
                            onClick={() => {
                              setHeaderSaved(false);
                              localStorage.setItem('currentInFacIssueHeaderSaved', 'false');
                            }}
                            title="Edit Header"
                            className="h-8 w-8 flex items-center justify-center rounded text-blue-600 bg-blue-50 border border-blue-200 hover:bg-blue-100/70 transition-all duration-200 shadow-sm hover:shadow"
                          >
                          ...
                          <button
                            ...
                            title="Delete Header"
                            className="h-8 w-8 flex items-center justify-center rounded text-red-600 bg-red-50 border border-red-200 hover:bg-red-100/70 transition-all duration-200 shadow-sm hover:shadow"
                          >
                        ...
                        <button
                          ...
                          disabled={isGenerating || (parentIssueId && headerSaved)}
