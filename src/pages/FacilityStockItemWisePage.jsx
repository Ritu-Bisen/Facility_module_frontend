...
export default function FacilityStockItemWisePage() {
  ...
  const loadStockData = async () => {
    ...
    setError(null);
    try {
      const result = await storeAPI.getFacilityStockItemWise(facilityId);
      setAllData(result || []);
      setDataLoaded(true);
    } catch (e) {
  ...
  const getFormattedData = () => filteredData.map((row, idx) => ({
    ...
    'Reserv Qty': row.RESERVQTY || row.reservqty || '0',
    'Group': row.GROUPNAME || row.groupname || '—',
    'EDL Type': row.EDLTYPE2025 || row.edltype2025 || '—',
    'CGMSC Flag': row.CGMSCFMFLAG || row.cgmscfmflag || '—',
  }));
  ...
  const exportToPDF = () => {
    ...
    doc.text('Facility Stock - Item Wise', 14, 15);
    autoTable(doc, {
              ...
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                <div>
                  <h1 className="text-xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 tracking-tight">
                    Facility Stock - Item Wise
                  </h1>
                  <p className="text-sm text-gray-500 mt-1 font-medium">View current stock levels grouped by drug (item-wise total)</p>
                </div>
                        ...
                        {filteredData.map((item, idx) => (
                          <tr key={idx} className="hover:bg-slate-50/60 align-middle">
                            ...
                            <td className="px-4 py-3 text-center">
                              <span className={`px-2 py-1 rounded-md text-xs font-bold ${(item.BATCHSTOCK || item.batchstock || 0) > 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'}`}>
                                {item.BATCHSTOCK || item.batchstock || 0}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-center text-xs text-slate-600">{item.RESERVQTY || item.reservqty || 0}</td>
                            <td className="px-4 py-3 text-xs text-slate-600">{item.GROUPNAME || item.groupname || '—'}</td>
                            <td className="px-4 py-3 text-xs">
                              <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${(item.EDLTYPE2025 || item.edltype2025) ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>
